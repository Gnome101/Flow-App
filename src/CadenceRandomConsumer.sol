...
abstract contract CadenceRandomConsumer {
     ...
     */
    function _getRevertibleRandomInRange(
        uint64 min,
        uint64 max
    ...
    function _requestRandomness() internal returns (uint256) {
        ...
        Request memory request = Request(
            CadenceArchUtils._flowBlockHeight(),
            block.number,
            false
        );
     ...
     */
    function _fulfillRandomRequest(
        uint256 requestId
     ...
     */
    function _fulfillRandomInRange(
        uint256 requestId,
        uint64 min,
        uint64 max
    ) internal returns (uint64) {
    ...
    function _fulfillRandomness(uint256 requestId) private returns (bytes32) {
        ...
        require(
            ...
            "Invalid request ID - value exceeds the number of existing requests"
        );
        ...
        bytes32 randomSource = CadenceArchUtils._getRandomSource(
            request.flowHeight
        );

        emit RandomnessSourced(
            requestId,
            request.flowHeight,
            request.evmHeight,
            randomSource
