...
library CadenceArchUtils {
    ...
    function _flowBlockHeight() internal view returns (uint64) {
        ...
        require(
            ok,
            "Unsuccessful call to Cadence Arch pre-compile when fetching Flow block height"
        );
    ...
    function _revertibleRandom() internal view returns (uint64) {
        ...
        require(
            ok,
            "Unsuccessful call to Cadence Arch pre-compile when fetching revertible random number"
        );
    ...
    function _getRandomSource(
        ...
        require(
            ok,
            "Unsuccessful call to Cadence Arch pre-compile when fetching random source"
        );
