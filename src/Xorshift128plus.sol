...
library Xorshift128plus {
    ...
    function seed(
        PRG memory prg,
        bytes memory sourceOfRandomness,
        bytes memory salt
    ) internal pure {
        require(
            sourceOfRandomness.length >= 16,
            "At least 16 bytes of entropy should be used when initializing the PRG"
        );
     ...
     */
    function _bigEndianBytesToUint64(
        bytes memory input,
        uint256 start
    ...
    function _requireNonZero(PRG memory prg) private pure {
        require(
            prg.state0 != 0 || prg.state1 != 0,
            "PRG initial state is 0 - must be initialized as non-zero"
        );
    }
}
