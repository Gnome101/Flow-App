...
import {MockCadenceRandomConsumer} from "../src/Mocks/MockCadenceRandomConsumer.sol";
import {Xorshift128plus} from "../src/Xorshift128plus.sol";

contract CadenceRandomConsumerTest is Test {
    MockCadenceRandomConsumer private consumer;
    ...
    event RandomnessRequested(
        uint256 requestId,
        uint64 flowBlockHeight,
        uint256 blockNumber
    );

    // Initialize the test environment
    function setUp() public {
        consumer = new MockCadenceRandomConsumer();
    ...
    function testFulfillRandomInRange() public {
        ...
        uint64 randomResult = consumer.fulfillRandomInRange(
            requestId,
            min,
            max
        );
        ...
        assertTrue(
            randomResult >= min && randomResult <= max,
