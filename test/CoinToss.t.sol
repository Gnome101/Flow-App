...
contract CoinTossTest is Test {
    ...
    function testFlipCoin() public {
        ...
        assertEq(
            coinToss.openRequests(requestId),
            1 ether,
            "Value amount sent should be 1 full FLOW"
        );
    }
    ...
    function testRevealCoinFailSameBlock() public {
        ...
        ); // Expect a revert since the block hasn't advanced
        coinToss.revealCoin();
    }
    ...
    function testRevealCoinWins() public {
        ...
        assertEq(
            finalBalance,
            initialBalance + expectedPrize,
            "User should have received a prize"
        );
        ...
        assertEq(
            false,
            hasOpenRequest,
            "User should not have an open request after revealing the coin"
        );
        ...
        assertEq(
            false,
            canFullfillRequest,
            "Request should not be fulfillable after revealing the coin"
        );
    }
    ...
    function testRevealCoinLoses() public {
        ...
        assertEq(
            finalBalance,
            initialBalance,
            "User should not receive winnings"
        );
    }
}
